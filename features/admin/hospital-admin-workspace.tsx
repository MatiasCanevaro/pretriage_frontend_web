...
type Invitation = { id: number; email: string; estado: string; roles: Role[]; venceEn: string; tokenEntregaUnica?: string | null };
...
export function HospitalAdminWorkspace({ hospitalId, hospitalName, userName }: { hospitalId: number; hospitalName: string; userName: string }) {
  ...
  function submitInvitation(event: FormEvent<HTMLFormElement>) {
    ...
    invite.mutate({ email: form.get("email"), roles: form.getAll("roles"), matricula: form.get("matricula") || null, especialidadIds: specialtyIds });
  }
      ...
      <div className="page-stack">
        ...
        <section className="panel">
          ...
          <form className="form-panel" onSubmit={submitInvitation}>
            <div className="form-grid"><label className="field">Correo<input name="email" type="email" required disabled={invite.isPending} /></label><label className="field">Matrícula médica<input name="matricula" disabled={invite.isPending} /></label><label className="field field-wide">IDs de especialidad (sólo médicos, separados por coma)<input name="specialtyIds" disabled={invite.isPending} /></label></div>
        ...
        <section className="panel"><div className="panel-heading"><div><h2>Invitaciones</h2><p>Pendientes, aceptadas, vencidas y revocadas.</p></div></div><div className="record-list">{query.data?.invitaciones.map((item) => <div className="record-row" key={item.id}><div><strong>{item.email}</strong><span>{item.roles.join(", ")} · {item.estado}</span></div>{item.estado === "PENDIENTE" ? <button className="button button-danger-ghost" disabled={revoke.isPending} onClick={() => revoke.mutate(item.id)}>Revocar</button> : null}</div>)}</div></section>
