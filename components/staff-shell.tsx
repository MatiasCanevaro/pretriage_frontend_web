...
export function StaffShell({ role, userName, section, sessionLabel, hospitalId, hospitalName, roles, canSwitchHospital, sectionNavigation, children }: StaffShellProps) {
    ...
    <div className="staff-app">
      ...
      <aside className={sectionNavigation ? "sidebar sidebar-section-links" : "sidebar"} aria-label="Navegación principal">
        <nav>
          ...
          {sectionNavigation ?? itemsByRole[role].map((item) => <span className={item === section ? "nav-item nav-item-active nav-subitem" : "nav-item nav-subitem"} key={item}><span className="nav-dot" aria-hidden="true" />{item}</span>)}
        </nav>
